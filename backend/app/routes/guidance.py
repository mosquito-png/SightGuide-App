...
async def describe(request: GuidanceRequest) -> GuidanceResponse:
    return await describe_scene(request.scene)


@router.post("/detect", response_model=VisionDetectionResponse)
async def detect(request: VisionDetectionRequest) -> VisionDetectionResponse:
...
async def read_text(request: OCRReadTextRequest) -> OCRReadTextResponse:
...
async def ask(request: VoiceAssistantQueryRequest) -> VoiceAssistantQueryResponse:
...
async def get_route(request: NavigationRouteRequest) -> NavigationRouteResponse:
...
async def search_place(request: PlaceSearchRequest) -> PlaceSearchResult:
